...
local md5, var_dump, c_GetTimeUS, c_GetRandom, c_GetServerID, c_RestrictAdminSessionIP = md5, var_dump, c_GetTimeUS, c_GetRandom, c_GetServerID, c_RestrictAdminSessionIP
local _SESSION, _REMOTE_IP = _SESSION, _REMOTE_IP

module("SessionModule")

local INVALID_SESSION_ID = "Invalid Session ID"
local root_dir = currentdir().."/session_admin"
local timeout = 1200
...
local function new_id ()
	return md5( (c_GetTimeUS() + c_GetRandom())..c_GetServerID().._REMOTE_IP )..md5(_REMOTE_IP)
end
...
function load (id)
	...
	if fileexist(filepath) then
		...
		if c_RestrictAdminSessionIP() == true and ipHash ~= md5(_REMOTE_IP) then
			return false
		end
