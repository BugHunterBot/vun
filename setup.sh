...
restart)
    ...
    ;;
*)
    echo "Usage: /etc/init.d/wftpserver [start|stop|restart]"
    exit 0
    ;;
esac
...
chmod +x "$INITD_WFTP" 1>/dev/null 2>&1


if [ -d "/etc/rc0.d" ]; then
set 0 1 6
for i in "$@"; do
	ln -s "$INITD_WFTP" "/etc/rc$i.d/K01wftpserver" 1>/dev/null 2>&1
done
set 2 3 4 5
for i in "$@"; do
	ln -s "$INITD_WFTP" "/etc/rc$i.d/S99wftpserver" 1>/dev/null 2>&1
done
else
set 0 1 6
for i in "$@"; do
	ln -s "$INITD_WFTP" "/etc/init.d/rc$i.d/K01wftpserver" 1>/dev/null 2>&1
done
set 2 3 4 5
for i in "$@"; do
	ln -s "$INITD_WFTP" "/etc/init.d/rc$i.d/S99wftpserver" 1>/dev/null 2>&1
done
fi
...
echo "Use the following command to start/stop WingFTP service: /etc/init.d/wftpserver [start|stop|restart]"
if [ -f "$SYSTEMD_WFTP" ]; then
	chmod +x "/etc/rc.d/rc.local" 1>/dev/null 2>&1
	echo "Or use systemctl command like this: systemctl [start|stop|restart] wftpserver.service"
fi
...
then
        if [ -f "$SYSTEMD_WFTP" ]; then
		systemctl start wftpserver.service 1>/dev/null 2>&1
        else
		cd $WING_HOME
		./wftpserver & 1>/dev/null 2>&1
        fi
